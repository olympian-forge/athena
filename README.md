...
_Named after [Athena](https://en.wikipedia.org/wiki/Athena), the Greek
...
[![Tests](https://img.shields.io/badge/Tests-499%20registered-success.svg)]()
...
_Note: The following results are from an actual run of `tools/perft.py` against this build, inside a Linux Docker container on an Early 2015 13-inch laptop (Dual-Core i7-5557U, 16GB RAM), single-threaded. Engine compiled with `-O3 -flto -march=native`. All 5 standard reference positions (Start Position, Kiwipete, and Positions 3-5) pass at every tested depth — see `PERFT_RESULTS.md` for the full table._
...
**`data_health.py`** — checks the _data_, not the network: decisive-game
...
**`diagnose_position.py`** — answers "why did the engine play _that_
...
the score with a confidence interval. This is a _relative_ measurement:
...
**`gauntlet_greedy.py`** and **`tactics.py`** — _absolute_ skill rungs
...
`export_onnx.py` needs an _existing_ `.pth`, and `train.py` only exports
